...
- `WatchlistService` — the orchestration layer every controller method actually calls into; owns the ownership check (`loadOwnedWatchlist`) used by every endpoint.
...
│   └── src/test/java/...       11 test classes (see §14)
├── frontend/
│   └── src/
